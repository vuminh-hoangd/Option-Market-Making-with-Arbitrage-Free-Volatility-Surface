...
$\psi_1(t,s) = \varphi(t,s) + \text{order-flow}$ is the maker's edge, it includes $\varphi \propto (\sigma^2 -\sigma^2_{\text{imp}})$
the difference of variances between the maker's believed volatility and the market-implied volatility, with $\sigma^2_{\text{imp}}(K,\tau)$ is sourced from the calibrated eSSVI implied volatility surface. Here $\psi_2(t) < 0$ is the inventory weight and inventory $𝑞$ does not change the total quoted width, only how it's split between bid and ask.
...
**Conclusion: Global eSSVI wins.** SVI's median is slightly better, but it comes from dropping the
