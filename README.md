...
Given a market maker who provides pricing quotes for a European call option with a given strike `K > 0` and maturity `τ > 0`, with `(S_t)_{t ≥ 0}` being the price process of the underlying risky asset. From the perspective of the market maker, `(S_t)_{t ≥ 0}` has the dynamics `dS_t / S_t = σ_t dB_t` where `B` is a standard one-dimensional Brownian motion. The market maker does not have any view on the (short-term) asset drift, while `(σ_t)_{t ≥ 0}` is their subjective assessment of the asset volatility process.
